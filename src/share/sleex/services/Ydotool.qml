...
Singleton {
    ...
    function releaseAllKeys() {
        ...
        Quickshell.execDetached(["bash", "-c", `${releaseCommand}`])
    ...
    function press(keycode) {
        Quickshell.execDetached(["bash", "-c", `ydotool key --key-delay 0 ${keycode}:1`]);
    }

    function release(keycode) {
        Quickshell.execDetached(["bash", "-c", `ydotool key --key-delay 0 ${keycode}:0`]);
    }
}
