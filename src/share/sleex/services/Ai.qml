...
Singleton {
    ...
    property var models: Config.options.policies.ai === 2 ? {} : {
        ...
        "mistral-medium-3": aiModelComponent.createObject(this, {
            ...
            "description": "Online | Mistral's model | Delivers fast, responsive and well-formatted answers. Disadvantages: not very eager to do stuff; might make up unknown function calls",
            ...
            "key_get_description": "**Instructions**: Log into Mistral account, go to Keys on the sidebar, click Create new key",
            "api_format": "mistral",
        }),
        "github-gpt-5-nano": aiModelComponent.createObject(this, {
            ...
            "api_format": "openai",
            "description": "Online via Github models | OpenAI's model",
    ...
    property string requestScriptFilePath: "/tmp/sleex/ai/request.sh"
    ...
    FileView {
        ...
        onLoadedChanged: {
            ...
            root.addMessage(`Loaded the following system prompt\n\n---\n\n${Config.options.ai.systemPrompt}`);
        }
    }

    function printPrompt() {
        root.addMessage(`The current system prompt is\n\n---\n\n${Config.options.ai.systemPrompt}`);
    }
    ...
    function printApiKey() {
        ...
        if (model.requires_key) {
            ...
            if (key) {
                root.addMessage(`API key: ${key}`, Ai.interfaceRole);
            } else {
    ...
    function handleFunctionCall(name, args: var, message: AiMessageData) {
        if (name === "switch_to_search_mode") {
            ...
            if (!args.key || !args.value) {
                addFunctionOutputMessage(name, "Invalid arguments. Must provide `key` and `value`.");
                return;
            }
            const key = args.key;
